...
function deleteReturnedLoans() {
  const loans = loadLoans();
  const returned = loans.filter((loan) => loan.status === "Devuelto");
  if (returned.length === 0) {
    alert("No hay registros con estado \"Devuelto\" para eliminar.");
    return;
  }
  if (!confirm(`¿Desea eliminar ${returned.length} registro(s) con estado "Devuelto"? Los préstamos activos no serán afectados.`)) return;
  saveLoans(loans.filter((loan) => loan.status !== "Devuelto"));
  renderLoans();
}
